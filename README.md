...
This writes `.github/workflows/border-collie-tick.yml` (the Orchestrator, which also runs Conflict and Refinement Workers inline) and `.github/workflows/border-collie-worker.yml` (one job per dispatched Worker), vendors the skills a Worker session invokes under `.claude/skills` along with the agent docs those skills read — never overwriting a file already there unless `--force` is passed — appends a `.border-collie/` entry to the repository's `.gitignore` (creating the file if there is none) so the fleet's own run logs never show up in your diffs, creates the tracker labels the loop reads and writes (`ready-for-agent`, `ready-for-human`, `border-collie:claimed`, `border-collie:operator-steered`), leaving any that already exist exactly as they are, then prints a checklist of the secrets and the minimum GitHub App permissions to supply before the first run (see "Continuous operation" below). Credentials come from a GitHub App you create and install yourself; border-collie hosts no shared service or webhook.

The skills are part of the scaffold because a Worker session invokes one by name. They used to be installed inside each Worker job from a plugin marketplace, which is a floating install on an unattended fleet — the very thing the pinned CLI install exists to prevent, since an upstream change lands in a sleeping fleet overnight. `init` writes the whole closure the Worker's `/implement` reaches instead, byte-identical to its upstream source (`docs/adr/0008-vendor-worker-skills-not-install-at-run-time.md`). Once written they are your repository's: edit them, add your own, delete what you don't want, and a re-run leaves every one of them alone. Only the `implement` skill's name is load-bearing — the Worker prompt invokes it by that name with no fallback.
